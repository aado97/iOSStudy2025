  ...
  var body: some View {
    NavigationStack {
      ScrollView {
        VStack(spacing: 24) {
          // Segmented Control
          Picker("기간 선택", selection: Binding(
            get: { viewModel.state.selectedPeriod },
            set: { viewModel.send(.updatePeriod($0)) }
          )) {
            ForEach([StatisticsViewModel.SelectedPeriod.day,
                     StatisticsViewModel.SelectedPeriod.week,
                     StatisticsViewModel.SelectedPeriod.month], id: \.self) { period in
              Text(period.rawValue).tag(period)
            }
          }
          .pickerStyle(.segmented)
          .padding(.horizontal, 24)
          .onChange(of: viewModel.state.selectedPeriod) {
          ...
          SectionView {
            HStack {
              VStack(alignment: .leading, spacing: 16) {
                Text("해당 \(viewModel.state.selectedPeriod.rawValue)의 포모도로")
                  .font(.system(size: 13))
                // ‼️뷰모델 작성시 수정 필요
              ...
              VStack(alignment: .leading, spacing: 16) {
                Text("해당 \(viewModel.state.selectedPeriod.rawValue)의 세션")
                  .font(.system(size: 13))
                // ‼️뷰모델 작성시 수정 필요
          ...
          SectionView {
            VStack {
              HStack {
                VStack(alignment: .leading, spacing: 16) {
                  Text("\(viewModel.state.selectedPeriod.rawValue)간 집중 시간")
                    .font(.system(size: 13))
          ...
          SectionView {
            HStack {
              VStack(alignment: .leading, spacing: 16) {
                Text("\(viewModel.state.selectedPeriod.rawValue)간 평균 세션")
                  .font(.system(size: 13))
              ...
              VStack(alignment: .leading, spacing: 16) {
                Text("\(viewModel.state.selectedPeriod.rawValue)간 평균 집중 시간")
                  .font(.system(size: 13))
